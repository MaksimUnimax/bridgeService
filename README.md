...
`BB2-DIRECT-01` принят: продуктовый, архитектурный и совместимый контракт зафиксирован в ветке `development`. Следующий ран — `BB2-DIRECT-02`. Direct ещё не production-ready. До `BB2-DIRECT-16` install-команда не публикуется. Раны выполняются строго по порядку; в `main` позднее переносится только чистый production release.
