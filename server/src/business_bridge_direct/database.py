...
def initialize_database(path: str) -> None:
    ...
    try:
        ...
        connection.execute("INSERT OR REPLACE INTO runtime_metadata(key, value) VALUES (?, ?)", ("service_version", "0.4.0"))
        connection.commit()
...
def check_database(path: str) -> bool:
    try:
        ...
        try:
            ...
            return version == SCHEMA_VERSION and table is not None and rows.get("schema_version") == "1" and rows.get("service_version") == "0.4.0"
        finally:
            connection.close()
