...
| BB2-DIRECT-10 | Connection bundle | Server one-line BB2D1 generation, deterministic artifact, production one-use pairing; browser checksum/version/expiry/canonicalization/P-256 parsing in real Chromium | `BB2-DIRECT-10-SRV_ACCEPTANCE_EVIDENCE.md`, `BB2-DIRECT-10-SRV_RESULTS.json`, `BB2-DIRECT-10-EXT_CHATGPT_EVIDENCE.md` | Server/extension | ACCEPTED / PASS | `BB2_DIRECT_10_COMPLETE` |
...
PASS: Direct `0.9.0`, schema `5`, emits a strict one-line `BB2D1` connection bundle containing the public endpoint, instance/server identity, one-time pairing session/code, issue/expiry timestamps and rotation generation with a domain-separated SHA-256 checksum. Attempt-7 acceptance passed 62/62 gates with deterministic wheel SHA-256 `50fefc54cf102e1081523dd548fb4c9709a559b50364760791d8eb8097ca21d2`; production pairing succeeded once and reuse was rejected.

The ChatGPT-owned browser parser validates exact framing/version, canonical unpadded base64url/JSON, checksum, TTL/expiry, canonical IPv4 and identifiers, P-256 SPKI and exact DER fingerprint without storage or network side effects. Node regression passed. Chromium `144.0.7559.96` secure-context Web Crypto acceptance passed 6/6 runs with 38/38 negative contract cases per run, including malformed/corrupt/canonicalization/expiry cases and P-384/RSA SPKI rejection. The repository shared-vector harness consumes the authoritative server fixture `tests/fixtures/BB2D1_BUNDLE_VECTORS.json`. Server runtime and Legacy were not changed by the extension step; `main` remained unchanged.
